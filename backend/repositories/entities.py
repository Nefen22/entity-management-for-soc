...
async def post_entity(tenant: str, type: str, value: str):
    type = type if type != "file-hashes" else "file_hashes"
    async with driver.session() as session:
        query ="""MERGE (node:{tenant}:{type}{{{key}: $value}}) RETURN node, labels(node) AS label""".format(tenant=TENANT_DATABASE[tenant],key=MAPPING_ENTITIES_KEY[type], type=MAPPING_ENTITIES_TYPE[type])
        result = await session.run(query, value=value)
        return await result.single()
